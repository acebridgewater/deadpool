...
Make sure you're in the project folder (the one with `package.json`), for example:
```bash
cd C:\Users\Administrator\Desktop\deadpool-master
```
```bash
npm install
```
...
This uses Electron Builder + NSIS to generate an installable `.exe`.
```bash
npm run dist:win
```
The installer will appear in the `dist/` folder.

> Note: If your environment rejects binary assets, omit custom icons (the default Electron icon will be used). You can add a `.ico` later when binary files are supported.

### One-click Windows build
If you want a single command that installs deps and builds the installer:
```bash
npm run oneclick:win
```

### Auto-updates (optional)
Auto-update checks are scaffolded but only run when you set a feed URL:
```bash
set DEADPOOL_UPDATES_URL=https://your-update-host/downloads
```
Then launch the app with `npm run start` and it will check for updates from that URL.
